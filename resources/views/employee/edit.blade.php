...
@section('content')
    <form method="POST" action="{{ route('employees.update', $employee->id) }}">
        ...
        <select id="wage_type" name="wage_type">
            <option value="hourly" {{ old('wage_type', $employee->wage_type) === 'hourly' ? 'selected' : '' }}>Hourly
            </option>
            <option value="salary" {{ old('wage_type', $employee->wage_type) === 'salary' ? 'selected' : '' }}>Salary
            </option>
        </select>
